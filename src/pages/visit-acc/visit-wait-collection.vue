<template>
  <section class="page visit-wait-collection">
    <data-form :model="waitModle" @onSearch="refreshData" :page="pageService" :buttonWrap="true" :rules="rules" @handleResetForm="follFeedback">
      <!--输入区域-->
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="5120">
          <el-input v-model="waitModle.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobileNo" v-auth="5121">
          <el-input v-model="waitModle.mobileNo" :maxlength="13"></el-input>
        </el-form-item>
      </template>
      <template slot="collapse-input">
        <el-form-item prop="idCard" label="身份证号" v-auth="5130">
          <el-input v-model="waitModle.idCard"></el-input>
        </el-form-item>
        <el-form-item label="批次号" prop="batchNumber" v-auth="5124">
          <el-input v-model="waitModle.batchNumber"></el-input>
        </el-form-item>
        <el-form-item label="机构" prop="deptCodes" v-auth="5125">
          <el-cascader v-model="waitModle.deptCodes" :options="departmentData" @change="getcupoName" :props="{value:'code',label:'name'}" change-on-select :show-all-levels="false" clearable style="line-height:1"></el-cascader>
        </el-form-item>
        <el-form-item label="案件标记" prop="caseMark" v-auth="5122">
         ...
         <el-form-item label="委托方" prop="principalId" v-auth="5123">
        ...
        <el-form-item label="当前催收员" prop="collectorName" v-auth="5126">
        ...
        <div class="inline-box">
          <el-form-item label="逾期金额" prop="caseMoney" v-auth="5131">
            <number-range v-model="waitModle.caseMoney"></number-range>
          </el-form-item>
        </div>
        <el-form-item label="逾期期数" prop="payStatus" v-auth="5129">
        ...
        <div class="inline-box">
          <el-form-item label="逾期天数" prop="dayRange" v-auth="5132">
         ...
         <el-form-item label="诉讼阶段" prop="follFeedbacks" v-auth="5007">
          <el-select  v-model="waitModle.lawsuitResult" placeholder="请选择" clearable>
      ...
      <template slot="default-button">
        <!--按钮区域-->
        <el-button @click="batchClick" v-auth="5133">批量分配</el-button>
        <el-button @click="leaveCaseClick" v-auth="5134">申请留案</el-button>
        <!-- <el-button @click="cancleleaveCase" v-auth="5135">取消留案</el-button> -->
        <!-- <el-button @click="applyFlowClick" v-auth="5136">提前流转</el-button> -->
        <el-button @click="circulationClick" v-auth="5136">申请流转</el-button>
    ...
    <data-box :data="waitDataSet" :selectionList.sync="selectionList" :rowClass="rowClass" @onPageChange="refreshData" :sort="sortService" :page="pageService" ref="data-box">
      <template slot="columns">
        <!--数据列区域-->
        <el-table-column label="操作" :min-width="$helper.getColumnWidth(6)">
          <template slot-scope="scope">
            <!-- <el-button @click="hangClick(scope)" type="text" v-if="scope.row.handUpFlag === 52" v-auth="5137">挂起</el-button>
            <el-button @click="unhangClick(scope)" type="text" v-if="scope.row.handUpFlag === 53" v-auth="5138">取消挂起</el-button> -->
            <el-button @click="repair=scope.row,dialog.FollowRecord=true" type="text" v-auth="5139">跟进明细</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column  label="流转状态" :min-width="$helper.getColumnWidth(4)">
         ...
         <el-table-column prop="caseFollowInTime" sortable='custom' label="案件流入时间" :min-width="$helper.getColumnWidth(5)">
          <template slot-scope="scope">
...
export default class VisitWaitCollection extends Vue {
   ...
   */
  feedbackChange() {
    this.waitModle.followupBack = this.waitModle.follFeedbacks[
      this.waitModle.follFeedbacks.length - 1
    ];
    if (!this.waitModle.followupBack) {
      this.waitModle.followupBack = "";
    }
  }
  /**
  ...
  confirmSend() {
    ...
    this.accVisitPoolService.batchVisitCase(sendData).subscribe(
      data => {
        ...
        this.$message.success(`成功分配${this.confirmData.allocated}个案件！`);
      },
      ({ msg }) => {
        this.$message.error(msg);
      }
