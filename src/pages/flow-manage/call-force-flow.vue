//电催强制流转
<template>
  <section class="page call-force-flow">
    <data-form :model="callForceModel" @onSearch="refreshData" :rules="rules">
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="5212">
          <el-input v-model="callForceModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobileNo" v-auth="5213">
          <el-input v-model="callForceModel.personalPhone" :maxlength="13"></el-input>
        </el-form-item>
      </template>
      <template slot="collapse-input">
        <el-form-item label="逾期金额" prop="amtRange" v-auth="5214">
          <number-range v-model="callForceModel.amtRange"></number-range>
        </el-form-item>
        <el-form-item label="逾期天数" prop="overdueDays" v-auth="5215">
          <number-range v-model="callForceModel.overdueDays" :isInteger="true"></number-range>
        </el-form-item>
        <el-form-item label="批次号" prop="batchNumber" v-auth="5216">
          <el-input v-model="callForceModel.batchNumber"></el-input>
        </el-form-item>
        <el-form-item label="委托方" v-auth="5217">
          <el-select v-model="callForceModel.principalId" placeholder="请选择" clearable>
            <el-option v-for="{index,id,name} in principalList" :key="index" :label="name" :value="id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="状态" prop="collectionStatus" v-auth="5218">
          <el-select v-model="callForceModel.collectionStatus" placeholder="请选择" clearable>
        ...
        <el-form-item label="身份证号" prop="idCard" v-auth="5219">
          <el-input v-model="callForceModel.idCard"></el-input>
        </el-form-item>
      </template>
      <template slot="default-button">
        <el-button @click="batchClick" v-auth="5220">批量分配</el-button>
      </template>
    </data-form>
    <data-box :data="callForceDataSet" :page="pageService" :selectionList.sync="selectionList" @onPageChange="refreshData">
    ...
    <!--批量分配-->
    <el-dialog title="请选择催收员" :center="true" :visible.sync="dialog.batchAllocate" @open="$nextTick(() => { $refs['inrush-redistribute'].refresh() })" width="70%">
    ...
    <!--案件分配失败-->
    <el-dialog title="案件分配失败" :visible.sync="dialog.errMsgOpen" :center="true" width="40%" :show-close="true" :close-on-click-modal="false" :close-on-press-escape="false">
...
export default class CallForceFlow extends Vue {
  ...
  private callForceDataSet = null;
  private callForceModel: any = {
    companyCode: "",
    personalName: "",
    personalPhone: "",
    orderId: "",
  ...
  get companyCode() {
    if (!this.isSupperAdmin) {
      this.callForceModel.companyCode = this.userData.companyCode;
    }
    return this.callForceModel.companyCode;
  }

  set companyCode(value) {
    this.callForceModel.companyCode = value;
  }
  /**
   * 页面激活
   */
  ...
  refreshData() {
    this.caseInfoService
      .electricForceCirculation(this.pageService, this.callForceModel)
      .subscribe(
        data => {
          this.callForceDataSet = data;
        },
        ({ msg }) => {
