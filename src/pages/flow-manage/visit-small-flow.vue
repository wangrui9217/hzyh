<template>
  <section class="page visit-small-flow">
    <data-form :model="visitSmallModel" :rules="rules" @onSearch="refreshData">
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="5221">
          <el-input v-model="visitSmallModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobileNo" v-auth="5222">
          <el-input v-model="visitSmallModel.personalPhone" :maxlength="13"></el-input>
        </el-form-item>
      </template>
      <template slot="collapse-input">
        <el-form-item label="逾期金额" prop="amtRange" v-auth="5223">
          <number-range v-model="visitSmallModel.amtRange"></number-range>
        </el-form-item>
        <el-form-item label="批次号" prop="batchNumber" v-auth="5224">
          <el-input v-model="visitSmallModel.batchNumber"></el-input>
        </el-form-item>
        <el-form-item label="逾期天数" prop="overdueDays" v-auth="5225">
          <number-range v-model="visitSmallModel.overdueDays" :isInteger="true"></number-range>
        </el-form-item>
        <el-form-item label="状态" prop="collectionStatus" v-auth="5226">
          <el-select v-model="visitSmallModel.collectionStatus" placeholder="请选择" clearable>
        ...
        <el-form-item label="委托方" v-auth="5227">
          <el-select v-model="visitSmallModel.principalId" placeholder="请选择" clearable>
            <el-option v-for="{index,id,name} in principalList" :key="index" :label="name" :value="id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="归属省份" prop="parentAreaId" v-auth="5228">
          <el-select v-model="visitSmallModel.parentAreaId" clearable>
            <el-option v-for="{value,label} in this.$city.getCityData({ level : 1 })" :key="value" :label="label" :value="value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="归属城市" prop="areaId" v-auth="5229">
          <el-select v-model="visitSmallModel.areaId" clearable>
            <el-option v-for="{value,label} in this.visitSmallModel['parentAreaId'] ? this.$city.getCityData({ level: 1, id: this.visitSmallModel['parentAreaId'] }) : []" :key="value" :label="label" :value="value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="身份证号" prop="idCard" v-auth="5230">
          <el-input v-model="visitSmallModel.idCard"></el-input>
        </el-form-item>
        <el-form-item label="流转状态" prop="caseType" v-auth="5231">
          <el-select v-model="visitSmallModel.caseType" placeholder="请选择" clearable>
      ...
      <template slot="default-button">
        <el-button @click="batchClick" v-auth="5232">批量分配</el-button>
      </template>
    </data-form>
    <data-box :data="visitSmallDataSet" :page="pageService" :selectionList.sync="selectionList" @onPageChange="refreshData">
      <template slot="columns">
        <el-table-column prop="caseNumber" label="申请号" :min-width="$helper.getColumnWidth(6)">
        </el-table-column>
        <el-table-column label="客户姓名" :min-width="$helper.getColumnWidth(3)" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{scope.row.personalInfo.name}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="personalInfo.mobileNo" label="手机号" :min-width="$helper.getColumnWidth(4)">
        </el-table-column>
...
export default class VisitSmallFlow extends Vue {
  ...
  private visitSmallDataSet = null;
  private visitSmallModel: any = {
    companyCode: "",
    personalName: "",
    personalPhone: "",
    batchNumber: "",
  ...
  get companyCode() {
    if (!this.isSupperAdmin) {
      this.visitSmallModel.companyCode = this.userData.companyCode;
    }
    return this.visitSmallModel.companyCode;
  }

  set companyCode(value) {
    this.visitSmallModel.companyCode = value;
  }
  /**
   * 页面激活
   */
  ...
  refreshData() {
    this.caseInfoService
      .outSmallCirculation(this.pageService, this.visitSmallModel)
      .subscribe(
        data => (this.visitSmallDataSet = data),
