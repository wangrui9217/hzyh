<template>
  <section class="page inrush-stripping">
    <data-form :model="closedModel" @onSearch="refreshData" :rules="inrushRule">
      <template slot="default-input">
        <el-form-item label="客户姓名" prop="personalName" v-auth="1446">
          <el-input v-model="closedModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobileNo" v-auth="1447">
          <el-input v-model="closedModel.mobileNo" :maxlength="13"></el-input>
        </el-form-item>
        <!--<el-form-item label="委托方" prop="principalId">-->
        <!--<el-select v-model="closedModel.principalId" placeholder="请选择" clearable>-->
        <!--<el-option v-for="{index,id,name} in principalList" :key="index" :label="name" :value="id">-->
        <!--</el-option>-->
        <!--</el-select>-->
        <!--</el-form-item>-->
      </template>
      <template slot="collapse-input" v-auth="1448">
        <el-form-item label="订单号" prop="orderId">
          <el-input v-model="closedModel.orderId"></el-input>
        </el-form-item>
        <el-form-item label="委案日期" prop="delegationDate" v-auth="1449">
          <el-date-picker class="daterange-large" range-separator="~" type="daterange" :editable="false" v-model="closedModel.delegationDate" placeholder="请选择日期" clearable>
          </el-date-picker>
        </el-form-item>
        <!--<el-form-item label="到期日期" prop="closeDate">-->
        <!--<el-date-picker class="daterange-large" range-separator="~" type="daterange" :editable="false" v-model="closedModel.closeDate"-->
        <!--placeholder="请选择日期" clearable>-->
        <!--</el-date-picker>-->
        <!--</el-form-item>-->
        <el-form-item label="逾期金额" prop="amtRange" v-auth="1450">
          <number-range v-model="closedModel.amtRange"></number-range>
        </el-form-item>
        <el-form-item label="逾期期数" prop="payStatus" v-auth="1451">
        ...
        <el-form-item label="逾期天数" prop="overdueDays" v-auth="1452">
          <number-range v-model="closedModel.overdueDays" :isInteger="true"></number-range>
        </el-form-item>
        <el-form-item label="门店" prop="shopDeptId" v-auth="1453">
    ...
    <data-box :page="pageService" :selectionList.sync="selectionList" :data="closedDataSet" @onPageChange="refreshData">
      <template slot="columns">
        ...
        <!--</el-table-column>-->
        <el-table-column prop="personalName" label="客户姓名" :min-width="$helper.getColumnWidth(6)" show-overflow-tooltip>
        </el-table-column>
  ...
  <span v-if="scope.row.assistFlag===1">是</span>
        ...
        <el-table-column label="跟进记录" :min-width="$helper.getColumnWidth(4)">
          <template slot-scope="scope">
  <el-button @click="followRecordClick(scope.row)" type="text" v-auth="1454">查看</el-button>
</template>
        </el-table-column>
      </template>
    </data-box>
...
export default class InrushStripping extends Vue {
  ...
  refreshData() {
    this.caseInfoReportService
      .queryStrippingCase(this.closedModel, this.pageService)
      .subscribe(
        data => {
