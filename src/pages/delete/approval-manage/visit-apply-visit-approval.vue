<template>
  <section class="page visit-apply-visit-approval">
    <data-form :model="assistModel" @onSearch="refreshData" :page="pageService" :rules="assistRule">
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="1528">
          <el-input v-model="assistModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="personalPhone" v-auth="1529">
          <el-input v-model="assistModel.personalPhone" :maxlength="13"></el-input>
        </el-form-item>
        <el-form-item label="逾期金额" prop="amtRange" v-auth="1530">
    ...
    <data-box :data="assistDataSet" @onPageChange="refreshData" :page="pageService">
      <template slot="columns">
        ...
        <el-table-column label="操作" :min-width="$helper.getColumnWidth(5)">
          <template slot-scope="scope">
            <el-button v-if="scope.row.approveStatus!==34" @click="assistObj=scope.row,dialog.check=true" type="text" size="small" v-auth="1532">查看</el-button>
            <el-button @click="assistObj=scope.row,dialog.approval=true" type="text" size="small" v-if="scope.row.approveStatus===34" v-auth="1531">审批</el-button>
            <el-button @click="assistObj=scope.row,dialog.record=true" type="text" size="small" v-auth="1533">跟进记录</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
    ...
    <el-dialog title="审批" :center="true" :visible.sync="dialog.approval" width="60%" @close="$refs['approval-form'].resetFields()">
      <div class="text-left">
        <el-form ref="approval-form" :model="approvalModel" :rules="approvalRules" label-width="100px">
          ...
          <el-form-item label="审批结果" prop="approveResult">
            <el-radio-group v-model="approvalModel.approveResult">
              <el-radio label="39">同意</el-radio>
              <el-radio label="38">拒绝</el-radio>
            </el-radio-group>
          </el-form-item>
        ...
        <el-row type="flex" justify="center">
          ...
          <el-button @click="nextStep" v-if="approvalModel.approveResult==='39'">下一步</el-button>
          <el-button @click="approvalCommit" :loading="submitLoading" v-else>确定</el-button>
        </el-row>
      </div>
    </el-dialog>
    <!--查看-->
      ...
      <div class="text-left">
        <el-form :model="assistObj" label-width="100px">
          ...
          <el-form-item label="审批结果">
            <el-radio-group v-model="assistObj.approveOutResult" disabled>
              <el-radio :label="39">同意</el-radio>
              <el-radio :label="38">拒绝</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="审批意见">
            <el-input type="textarea" v-model="assistObj.approveOutMemo" readonly></el-input>
          </el-form-item>
        </el-form>
      </div>
    </el-dialog>
    ...
    <!--案件分配弹框-->
    <el-dialog title="请选择催收员" :center="true" :visible.sync="dialog.manual" width="80%" @open="$nextTick(()=>$refs['manual-distrubute'].refresh(assistObj, approvalModel))">
      <manual-distrubute ref="manual-distrubute" @close="dialog.manual=false" @refreshList="dialog.manual=false,dialog.approval=false,refreshData()"></manual-distrubute>
    </el-dialog>
  </section>
</template>
...
import ManualDistrubute from "~/components/approval-manage/visit-apply-visit-approval/manual-distrubute.vue";
...
export default class VisitApplyVisitApproval extends Vue {
...
*/
  ...
  refreshData() {
    this.caseOutboundApplyService.findAllPassedApply(this.assistModel, this.pageService, { applyDate: 'desc' }).subscribe(data => {
