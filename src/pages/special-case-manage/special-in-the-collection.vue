<template>
  <!-- 特殊案件管理-催收中案件 --> 
  <section class="page special-in-the-collection">
    ...
    <el-dialog title="案件申请流转" width="40%" :visible.sync="dialog.circulation" :center="true" :close-on-click-modal="false" :close-on-press-escape="false" @close="$refs['application-circulation'].reset()">
      <application-circulation :selectFilter="856" :caseNumber="circulationCaseId" @close="dialog.circulation=false;refreshData();" ref="application-circulation"></application-circulation>
    </el-dialog>
    <!-- 案件详情 -->
    ...
    <el-dialog :title="changeTitle" :center="true" :visible.sync="dialog.caseAllot" append-to-body width="80%" >
      <case-allot ref="case-allot" :modelType="[508]" @parentTitle="parentTitle" @close="dialog.caseAllot = false" @success="refreshData();dialog.caseAllot = false"></case-allot>
    </el-dialog>
  </section>
</template>
...
export default class SpecialInTheCollection extends Vue {
  ...
  @specialManageModule.State allotCase;
