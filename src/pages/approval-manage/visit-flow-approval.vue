<template>
  <section class="page vistit-flow-approval">
    <data-form :model="assistModel" @onSearch="refreshData" :page="pageService">
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="5290">
          <el-input v-model="assistModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="审批结果" prop="approveState" v-auth="5291">
      ...
      <template slot="columns">
        <el-table-column label="操作" :min-width="$helper.getColumnWidth(5)">
          <template slot-scope="scope">
            <el-button @click="assistObj=scope.row,dialog.approval=true" type="text" size="small" v-if="scope.row.approveResult===213" v-auth="5292">审批</el-button>
            <el-button @click="assistObj=scope.row,dialog.record=true" type="text" size="small" v-auth="5293">跟进记录</el-button>
          </template>
        </el-table-column>
    ...
    <!--电催流转审批-->
    <el-dialog title="外访流转审批" :center="true" :visible.sync="dialog.approval" width="40%" @close="$refs['approval-form'].resetFields()">
      <section class="component approval-vistit-flow-application">
        <el-form ref="approval-form" :model="approvalModel" :rules="approvalRules" label-width="100px">
          <el-form-item label="申请原因" prop="applyReason">
            <el-input type="textarea" v-model="assistObj.applayReason"></el-input>
          </el-form-item>
...
export default class VistitFlowApproval extends Vue {
  ...
  */
  ...
  refreshData() {
    this.accVisitPoolService
      .getVisitPendingCase(this.assistModel, this.pageService, {
        approveResult: "asc",
        applayDate: "desc",
