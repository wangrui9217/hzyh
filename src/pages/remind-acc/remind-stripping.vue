    ...
    <data-box :page="pageService" :selectionList.sync="selectionList" :data="closedDataSet" @onPageChange="refreshData">
      <template slot="columns">
        ...
        <el-table-column label="跟进记录" :min-width="$helper.getColumnWidth(4)">
          <template slot-scope="scope">
            <el-button @click="followRecordClick(scope.row)" type="text" v-auth="1416">查看</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
...
<script lang="ts">
  ...
  import {
    CaseInfoReportService
  } from "~/services/report-service/case-info-report.service";
  import {
    ExportFollowupService
  } from "~/services/report-service/export-followup.service";
  ...
  export default class RemindStripping extends Vue {
    ...
    refreshData() {
      this.caseInfoReportService.queryStrippingCase(this.closedModel, this.pageService).subscribe(data => {
