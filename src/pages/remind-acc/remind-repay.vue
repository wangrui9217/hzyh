<template>
  <section class="page remind-repay">
    <data-form :model="closedModel" @onSearch="refreshData" :rules="inrushRule">
      <template slot="default-input">
        ...
        <el-form-item label="客户姓名" prop="personalName" v-auth="1401">
          <el-input v-model="closedModel.personalName"></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="mobileNo" v-auth="1402">
          <el-input v-model="closedModel.mobileNo" :maxlength="13"></el-input>
        </el-form-item>
      </template>
      <template slot="collapse-input">
        <el-form-item label="逾期金额" prop="amtRange" v-auth="1405">
          <number-range v-model="closedModel.amtRange"></number-range>
        </el-form-item>
        <el-form-item label="逾期天数" prop="overdueDays" v-auth="1403">
          <number-range v-model="closedModel.overdueDays" :isInteger="true"></number-range>
        </el-form-item>
        <el-form-item label="门店" prop="shopDeptId" v-auth="1407">
      ...
      <template slot="columns">
        ...
        <el-table-column prop="caseNumber" label="申请号 " :min-width="$helper.getColumnWidth(6)">
          <template slot-scope="scope">
            <el-button type="text" @click="dialog.detail=true, followObj=scope.row">{{scope.row.caseNumber}}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="跟进记录" :min-width="$helper.getColumnWidth(4)">
          <template slot-scope="scope">
            <el-button @click="followRecordClick(scope.row)" type="text" v-auth="1408">查看</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
...
export default class RemindRepay extends Vue {
  ...
  refreshData() {
    this.caseInfoReportService.queryLatelyPayCase(this.closedModel, this.pageService).subscribe(data => {
