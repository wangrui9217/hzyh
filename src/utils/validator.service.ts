...
export class ValidatorService {
   ...
   * 验证手机和电话号码
   */
  static phoneAndCallNumber(rule, value, callback) {
    if (ValidatorService.regex.callNumber.test(value) || ValidatorService.regex.phoneNumber.test(value) || !value) {
      callback();
    } else {
      callback(new Error("请输入正确的电话或手机"));
    }
  }
