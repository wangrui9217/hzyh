<template>
  <section class="component add-foreign-client">
    <el-form :rules="rules" :model="forms" ref="add-form" label-width="100px" class="text-left">
        ...
        <el-col :span="12">
          <el-form-item label="机构类型" prop="outsOrgtype">
            <el-radio-group v-model="forms.outsOrgtype">
              <el-radio v-for="{value,label} in this.$dict.getDictData('0036')" :label="value" :key="value">{{label}}</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-form-item label="可催收区域" prop="areaList">
        ...
        <span v-show="forms.areaList.length" style="padding:5 0 0 5px;color:red;">{{`您选择了${forms.areaList.length}个城市`}}</span>
      </el-form-item>
...
export default class AddForeignClient extends Vue {
