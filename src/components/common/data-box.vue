<template>
  <section class="component data-box">
    <div class="table">
      <el-table :row-style="rowStyle" :row-class-name="rowClass" :emptyText="emptyText" :height="tableHeight()" :max-height="maxHeight" tooltip-effect="dark" ref="table" :data="data" border stripe :highlight-current-row="selectionRow!=undefined" @current-change="onCurrentChange" @selection-change="onSelectionChange" @sort-change="onSortChange">
        <el-table-column v-if="selectionList" type="selection" width="40">
        </el-table-column>
        <el-table-column v-if="!hiddenIndex" type="index" :label="indexLabel" width="50" align="center">
        </el-table-column>
        <slot name="columns"></slot>
      </el-table>
    </div>
    <div v-if="page&&data&&data.length>0" class="page row end-span">
      <el-pagination size="small" :layout="page.layout" :total="page.total" :current-page.sync="page.pageIndex" :page-size="page.pageSize" :page-sizes="page.pageSizeOpts" @size-change="pageSizeChange" @current-change="pageIndexChange"></el-pagination>
    </div>
  </section>
</template>
