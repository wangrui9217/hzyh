<template>
  <section class="component send--group-message">
    <el-col>
      <el-col>
...
export default class SendGroupMessage extends Vue {
  ...
  refreshData(val) {
    ...
    let getPhoneData: any = {
      personalId: this.caseInfo.personalInfo
        ...
        : "",
      idCard: this.caseInfo.personalInfo
        ? this.caseInfo.personalInfo.idCard
        : ""
    };
  ...
  templateClassChange() {
    ...
    this.templateService.getTemplatesByCondition(module).subscribe(
      data => {
        let TemplateData = data.content.filter(
          v => v.templateType === Number(this.templateClass)
        );
        if (TemplateData) {
