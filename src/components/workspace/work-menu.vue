<template>
  <section class="component work-menu" :class="[{collapse:menuCollapse}]">
    ...
    <el-menu unique-opened :default-active="$route.fullPath" @select="selectMenuItem">
      <work-menu-item v-for="item in menuList" :data="item" :key="item.id" @select="selectMenuItem"></work-menu-item>
    </el-menu>
  </section>
</template>
...
export default class WorkMenu extends Vue {
   ...
   */
  get menuList() {
    let rescource = this.menuResource.filter(x => [17, 18].includes(x.type)&&x.url&&!x.show);

    let menus = rescource
      .filter(x => x.type === 17)
      .map(item => {
        let children = rescource
          .filter(x => x.parentId === item.id && x.type === 18&&x.url&&!x.show)
