  ...
  <section class="component follow-record">
    ...
    <data-form :model="model" @onSearch="refreshData" :page="pageService" @handleResetForm="handleResetForm">
      <template slot="default-input">
        <el-form-item label="跟进日期" prop="operatorTime">
          <el-date-picker v-model="model.operatorTime" type="daterange" :editable="false">
          </el-date-picker>
        </el-form-item>
    ...
    <data-box :data="dataSet" @onPageChange="refreshData" :page="pageService">
      <template slot="columns">
        ...
        <el-table-column label="电话/地址" :min-width="$helper.getColumnWidth(3)">
          <template slot-scope="scope">
            <span v-if="scope.row.type === 78">{{scope.row.contactPhone}}</span>
            <span v-else>{{scope.row.detail}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="collectionFeedback" label="催收反馈" :min-width="$helper.getColumnWidth(5)">
          <template slot-scope="scope">
  ...
  refreshData() {
    ...
    if (this.out) {
      ...
      this.caseInfoService
        .getCaseInfoFollowRecord(this.model, this.pageService, {
          operatorTime: "desc"
        })
        .subscribe(
          data => {
